...
namespace Dinwlooc.Common.Bridge
{
    ...
    /// </summary>
    public class SlideBridge : ISlideBridge
    {
        private static SlideBridge? _instance;
        public static SlideBridge Instance => _instance ??= new SlideBridge();
        private SlideBridge() { }
        ...
        public void SetSlideDecay(float decay)
        {
            ...
            PlayerController? ctrl = GetController();
            if (ctrl == null) return;
            ctrl.SlideDecay = Mathf.Max(0f, decay);
        }
    }
}