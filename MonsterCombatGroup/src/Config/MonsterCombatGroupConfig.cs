...
namespace MonsterCombatGroup
{
    public class MonsterCombatGroupConfig : ConfigBase<MonsterCombatGroupConfig>
    {
        ...
        public override void Bind(ConfigFile config)
        {
            ...
            EnableLeaderMechanic = config.Bind("General", "Enable", true, "启用领队机制");
            ...
            LeaderHealthMultiplier = config.Bind("Stats", "LeaderHealthMultiplier", 15f,
                new ConfigDescription("领队生命值倍率", new AcceptableValueRange<float>(1f, 100f)));
            GuardHealthMultiplier = config.Bind("Stats", "GuardHealthMultiplier", 5f,
                new ConfigDescription("护卫生命值倍率", new AcceptableValueRange<float>(1f, 50f)));
            ...
            EnableBatteryDrainOnLeaderHurt = config.Bind("Effects", "EnableBatteryDrain", true, "领队受伤减少玩家电量");
            EnableGuardStunRecoveryOnHurt = config.Bind("Effects", "EnableGuardStunRecovery", true, "护卫受伤解除眩晕");

            CommandInterval = config.Bind("Combat", "CommandInterval", 5f,
                new ConfigDescription("指挥状态下召集攻击间隔（秒）", new AcceptableValueRange<float>(1f, 30f)));
            CommandAttackCount = config.Bind("Combat", "CommandAttackCount", 6,
                new ConfigDescription("每次锁定时指挥攻击次数", new AcceptableValueRange<int>(1, 20)));
            GlobalStunImmunityDuration = config.Bind("Immunity", "GlobalStunImmunity", 1f,
                new ConfigDescription("指挥状态下全体怪物受击获得的眩晕免疫时长（秒）", new AcceptableValueRange<float>(0f, 10f)));
            LeaderExtraStunImmunityPerGuard = config.Bind("Immunity", "LeaderExtraPerGuard", 4f,
                new ConfigDescription("每名护卫为领队额外提供的眩晕免疫时长（秒）", new AcceptableValueRange<float>(0f, 20f)));

            EnableLeaderDeathReward = config.Bind("Reward", "Enable", true, "启用领队死亡奖励（Valuable 无敌保护）");
