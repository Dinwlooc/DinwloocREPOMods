...
- 体力加速恢复**仅对本地玩家有效**，不影响其他客户端（但配置可由房主统一）。
- 若开启 `UseHostConfig`：
  - **房主**：模组加载时及每次有玩家进入关卡时，自动广播当前体力配置。
  - **客户端**：忽略本地 `Percent`、`CompensateWhenDisabled`、`EnableCrouchBoost` 设置，等待房主广播。若未收到广播（如房主未开启该功能或网络延迟），体力加速**不会生效**，直到收到配置。
...
- 体力加速恢复仅影响**本地玩家**，多人模式下各自独立（除非开启房主配置同步）。
...
- 多人模式下，充电、自愈、复活操作由房主执行并同步，体力加速仅本地生效。
